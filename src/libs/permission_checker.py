...
async def Permission_checker(username, input_permission_code):
    ...
    try:
        user_role = Permission.find_one(Permission.perrmission_code == input_permission_code)
        if not user_role:
            new_per : Permission = Permission(perrmission_code=input_permission_code, permission_descripition="")
            await new_per.insert()

        current_user : User = User.find_one(User.username==username)
        if not current_user:
            return False
        
    
        # current_role : Role = Role.find_one(Role.id == ObjectId(current_user.role_id))
        if PermissionRole.find_one(PermissionRole.role_name==current_user.role_id,
                                    PermissionRole.permission_code=="*"):
            return True
        
    
        if not PermissionRole.find_one(PermissionRole.role_name==current_user.role_id,
